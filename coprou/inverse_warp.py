...
def inverse_warp2(img, depth, ref_depth, ref_unty, pose, intrinsics, padding_mode='zeros'):
    """
    Inverse warp a source image to the target image plane.
    Args:
        img: the source image (where to sample pixels) -- [B, 3, H, W]
    ...
    cam_coords = pixel2cam(depth.squeeze(1).contiguous(), intrinsics.inverse())  # [B,3,H,W]
    ...
    ref_unty_warped = F.grid_sample(ref_unty, src_pixel_coords, padding_mode=padding_mode, align_corners=False)
