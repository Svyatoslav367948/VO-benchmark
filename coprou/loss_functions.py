...
def compute_pairwise_loss(tgt_img, ref_img, tgt_depth, ref_depth, pose, intrinsic, with_ssim, with_mask, with_auto_mask, padding_mode):
    ...
    tgt_depth, tgt_unty = tgt_depth
    ref_depth, ref_unty = ref_depth

    ref_img_warped, valid_mask, projected_depth, computed_depth, ref_unty_warped = inverse_warp2(ref_img, tgt_depth, ref_depth, ref_unty, pose, intrinsic, padding_mode)
    ...
    diff_depth = diff_depth * (1 - combined_unty).detach() 
...
def mean_on_mask(diff, valid_mask):
    ...
    precentage = valid_mask.sum() / mask.size(0) / diff.size(-1) / diff.size(-2)
    if precentage > 0.05:
...
def compute_smooth_loss(tgt_depth, tgt_img, ref_depths, ref_imgs):
    ...
    loss = get_smooth_loss(tgt_depth[0][0], tgt_img)

    for ref_depth, ref_img in zip(ref_depths, ref_imgs):
        loss += get_smooth_loss(ref_depth[0][0], ref_img)
