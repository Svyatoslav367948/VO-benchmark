...
class KittiEvalOdom():
    ...
    def plot_trajectory(self, poses_gt, poses_result, seq):
        ...
        fig_base = self.plot_path_dir + "/" + png_title

        # Save as PDF
        plt.savefig(fig_base + ".pdf", format='pdf', bbox_inches='tight', pad_inches=0)

        # Save as SVG
        plt.savefig(fig_base + ".svg", format='svg', bbox_inches='tight', pad_inches=0)
        plt.close(fig)
    ...
    def multi_plot_trajectory(self, poses_dict, seq):
        ...
        style_dict = {
            "Ground Truth": {"color": "black", "linestyle": "--", "linewidth": 3},
            "Ours-1": {"color": "red", "linestyle": "-", "linewidth": 3},
            "Ours-2": {"color": "blue", "linestyle": "-", "linewidth": 3},
            "SC-Depth": {"color": "lightgray", "linestyle": "-", "linewidth": 3},
        ...
        plt.legend(loc="upper left", prop={'size': fontsize_ - 5})
        plt.xticks(fontsize=fontsize_)
        plt.yticks(fontsize=fontsize_)
        plt.xlabel('x (m)', fontsize=fontsize_)
        plt.ylabel('z (m)', fontsize=fontsize_)
        fig.set_size_inches(10, 10)
        png_title = "sequence_{:02}".format(seq)
        fig_base = self.plot_path_dir + "/" + " multi_" + png_title
    ...
    def eval(self, gt_dir, result_dir,
        ...
        for i in self.eval_seqs:
            ...
            poses_predictions = {
                "DF-VO": poses_dfvo,
                "KPDepth-VO": poses_kpdvo,
                "SC-Depth": poses_sc_sfmlearner,
                "Ours-1": poses_result,
                "Ours-2": poses_uvo2,
                # "SfMLeaner": poses_sfmlearner
            }
