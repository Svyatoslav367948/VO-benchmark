...
def load_tensor_image(filename, args):
    ...
    if (not args.no_resize) and (h != args.img_height or w != args.img_width):
        ...
        img = img.resize((args.img_width, args.img_height), Image.ANTIALIAS)
...
#     assert tensor_img1.device == tensor_img2.device, "Input tensors must be on the same device"
#     assert tensor_img1.device.type == 'cuda', "This function uses CUDA timing, inputs must be on GPU"
...
#     for _ in range(10):
#         _ = pose_net(tensor_img1, tensor_img2)
...
#         with torch.no_grad():
#             _ = pose_net(tensor_img1, tensor_img2)
#             torch.cuda.synchronize()  # ensure the kernel completes per iteration

#     end_event.record()
#     torch.cuda.synchronize()

#     elapsed_time_ms = start_event.elapsed_time(end_event)  # Total time in milliseconds
#     avg_time_s = elapsed_time_ms / n_iters / 1000          # Average time per inference in seconds
#     fps = 1.0 / avg_time_s

#     print(f"[Inference FPS] Average over {n_iters} iterations: {fps:.2f} FPS")
#     return fps
